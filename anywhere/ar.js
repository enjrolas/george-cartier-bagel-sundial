...
function buildMarkers() {
  ...
  shadowMarker.appendChild(createMatrixTriangle(104, 190)); // glowing matrix-bagel pointer
  const stag = document.createElement('span'); stag.className = 'tag'; stag.textContent = 'shadow';
  shadowMarker.appendChild(stag);
...
function buildSundial(THREE) {
  ...
  // shadow ray (updated live) — matrix green
  const ray = new THREE.Mesh(
    new THREE.BoxGeometry(0.03, 0.01, R).translate(0, 0.01, -R / 2),
    new THREE.MeshBasicMaterial({ color: 0x2bff86 })
  );
